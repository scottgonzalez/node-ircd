...
User.prototype.sendMessage = function(msg, from) {
	...
	var prefix = from ? from.prefix() : this.server.name,
		packet = ":" + prefix + " " + msg + "\r\n";
...
User.prototype.join = function(channelName) {
	var channelName = Channel.normalizeName(channelName);
	...
	var channel = lookupChannel(this.server, channelName);
...
function maybeRegister(user) {
	if (user.nick && user.names && !user.registered) {
		user.sendMessage("001 " + user.nick + " :Welcome to " + user.server.name);
...
User.prototype.privmsg = function (target, msg) {
	if (target.charAt(0) == "#") {
		var channelName = Channel.normalizeName(target);
		for (var i = 0; i < this.channels.length; i++) {
	...
	switch (command) {
		...
		case "NAMES":
			...
			for (var i = 0; i < channelNames.length; i++) {
				var channelName = Channel.normalizeName(channelNames[i]);
				if (channels[channelName]) {
		...
		case "WHO":
			var args = rest.split(/\s/);
			var channelName = Channel.normalizeName(args[0]);
			if (channels[channelName]) {
		...
		case "PING":
			...
			this.sendMessage("PONG " + this.server.name);
		break;
...
function lookupChannel (server, name) {
	if (!(name in channels)) {
		channels[name] = new Channel(server, name, "node.js ircd");
	}
